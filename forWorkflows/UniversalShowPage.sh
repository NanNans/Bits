...
echo "$1" | sed -E 's/[0-9]+(.+)/\1/' | sed -E 's/([[:alnum:][:punct:]]) $/\1/' | sed -E 's/[[:punct:]]//g' #cuts down to only stuff after digit to extract ep name, then gets rid of trailing space and punctuation
