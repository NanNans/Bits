...
AFTERID=$(echo "$CLEAN1" | sed -E 's/.*s[0-9]+e[0-9]+([[:alnum:][:punct:] ]+)/\1/' | sed -E 's/[[:punct:]]//g' | sed -E 's/[0-9][0-9][0-9][0-9] *$//' | sed -E 's/ $//')
...
echo "$FOLDERQUERIED"
if [[ $FOLDERQUERIED =~ $BEFOREID ]] ; then
	mv "$1" "$dir"
	echo "GoodRun"
else
	osascript -e "tell application \"Finder\" to set label index of alias POSIX file \"$1\" to \"5\""
fi
done
